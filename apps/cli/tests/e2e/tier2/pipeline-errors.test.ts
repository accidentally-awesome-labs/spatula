...
describe('LLM failure handling', () => {
  ...
  it(
    'classifier malformed JSON -> pipeline continues',
    async (ctx) => {
      if (!playwrightOk) {
        ctx.skip();
        return;
  ...
  it(
    'extractor malformed JSON -> extraction skipped, pipeline continues',
    async (ctx) => {
      if (!playwrightOk) {
        ctx.skip();
        return;
  ...
  it(
    'schema evolution Zod failure -> no broken actions in DB',
    async (ctx) => {
      if (!playwrightOk) {
        ctx.skip();
        return;
  ...
  it(
    'all LLM calls fail -> pages stored, zero entities, run completed',
    async (ctx) => {
      if (!playwrightOk) {
        ctx.skip();
        return;
  ...
  it(
    'zero unmapped fields -> field-proposer never called or returns early',
    async (ctx) => {
      if (!playwrightOk) {
        ctx.skip();
        return;
