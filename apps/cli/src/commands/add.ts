...
export function formatAddResult(result: AddResult): string {
  ...
  if (result.added.length > 0) {
    ...
    for (const url of result.added) lines.push(`  + ${url}`);
  }
  if (result.duplicates.length > 0) {
    lines.push(`Skipped ${result.duplicates.length} duplicate(s):`);
    for (const url of result.duplicates) lines.push(`  ~ ${url}`);
  }
  ...
  if (result.invalid.length > 0) {
    ...
    for (const url of result.invalid) lines.push(`  ✗ ${url}`);
  }
  if (
    result.added.length === 0 &&
    result.duplicates.length === 0 &&
    result.invalid.length === 0 &&
    result.alreadyCrawled.length === 0
  )
