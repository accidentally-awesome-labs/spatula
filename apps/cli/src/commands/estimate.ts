...
import { findProjectRoot, parseProjectYaml, yamlToJobConfig, loadGlobalConfig, estimateCost } from '@spatula/core';
...
export function formatCostEstimate(estimate: CostEstimate): string {
  ...
  for (const entry of estimate.llmCallBreakdown) {
    ...
    lines.push(`  ${entry.purpose.padEnd(22)} ${model.padEnd(18)} ${String(entry.calls).padEnd(8)} $${entry.costUsd.toFixed(3)}`);
  }
  if (estimate.warnings.length > 0) { lines.push('\n  Warnings'); for (const w of estimate.warnings) lines.push(`  ! ${w}`); }
  return lines.join('\n');
}
...
export async function runEstimateCommand(): Promise<void> {
  ...
  const jobConfig = yamlToJobConfig(projectYaml, { tenantId: projectId, projectId, projectRoot, globalConfig });
