...
_Last updated: 2026-05-20 after completing Phase 17 (Browser Auth, SSE, CORS) — SSE job-event stream with Last-Event-ID resume + single-use stream tokens, wildcard-subdomain CORS, Dex OIDC example kit, zero-downtime API key rotation, OpenAPI-driven cross-tenant isolation audit, M2M client_credentials e2e; all 8 AUTH requirements verified, browser + M2M e2e passing live. A fail-open JWT-scope regression was caught by the regression gate and fixed (auth is fail-closed; M2M/browser scopes are explicit opt-in grants)._
