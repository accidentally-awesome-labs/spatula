...
- **D-03:** **Full live measurement this phase.** Run the 1k-page crawl **live, once per tier**, on the one defined VM (3 live 1k-page runs total — real LLM spend + crawl wall-clock). The result is a genuinely *measured* table, not synthetic. Document the harness + assumptions so self-hosters can re-run for their own hardware.
...
- **Release-workflow launch polish + post-publish smoke + cosign-in-launch-runbook** — Phase 22. Phase 19 ships the signing *infrastructure*; Phase 22 polishes/operationalizes it.
