...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - **Drift detection positive-failure proof (D-14):** Drift detection at PR time is proved by the dev-only boot validator in 16-3 Task 1 (`validateExamplesAtBoot`) — that block fails app boot when any OpenAPI example is off-schema, and 16-3 Task 1's openapi.test.ts asserts the validator returns errors on a deliberately bad fixture (positive failure case). The contract suite re-runs the SAME Ajv2020 validation against ACTUAL responses (not just declared examples), so the suite catches BOTH spec drift (validator returns errors) AND runtime drift (handler returns wrong shape). This is the belt-and-suspenders pattern from CONTEXT.md D-07: dev-time boot validator (16-3) + PR-time contract suite (16-4). The positive-failure proof at the validator level is verified by 16-3 Task 1's openapi.test.ts; the contract suite itself is implicitly verified by green-path execution against the seeded fixtures plus the fact that the validator + matrix driver share the same Ajv2020 compile path (see `tests/contract/helpers/ajv-setup.ts` + `apps/api/src/openapi-config.ts` validateExamplesAtBoot — both use `Ajv2020({ strict: false, allErrors: true })`).
    - Cross-link verification: `grep -q "validateExamplesAtBoot" apps/api/src/openapi-config.ts` (16-3 Task 1 still in place) AND `grep -q "from 'ajv/dist/2020" tests/contract/helpers/ajv-setup.ts` (16-4 Task 1 same Ajv2020 import).
