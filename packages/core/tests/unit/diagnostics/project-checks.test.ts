...
describe('createProjectChecks', () => {
  it('returns 8 project checks', () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/test', validateYaml: vi.fn().mockReturnValue(true) });
  ...
  it('spatula-yaml check fails for invalid config', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/test', validateYaml: vi.fn().mockImplementation(() => { throw new Error('Invalid YAML'); }) });
    const result = await checks.find((c) => c.name === 'spatula-yaml')!.run();
  ...
  it('db-integrity check warns when no DB exists', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/nonexistent-dir-12345', validateYaml: vi.fn() });
  ...
  it('db-integrity check passes with custom checker returning ok', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/nonexistent-dir-12345', validateYaml: vi.fn(), checkDbIntegrity: vi.fn().mockResolvedValue({ ok: true, message: 'OK' }) });
    const result = await checks.find((c) => c.name === 'db-integrity')!.run();
    expect(result.status).toBe('warn'); // file doesn't exist
  });

  it('orphaned-tasks check returns warn when orphaned tasks exist', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/test', validateYaml: vi.fn(), getOrphanedTaskCount: vi.fn().mockResolvedValue(3) });
  ...
  it('orphaned-tasks check passes when no orphaned tasks', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/test', validateYaml: vi.fn(), getOrphanedTaskCount: vi.fn().mockResolvedValue(0) });
    const result = await checks.find((c) => c.name === 'orphaned-tasks')!.run();
    expect(result.status).toBe('pass');
  });

  it('pending-actions check returns warn with count', async () => {
    const checks = createProjectChecks({ projectRoot: '/tmp/test', validateYaml: vi.fn(), getPendingActionCount: vi.fn().mockResolvedValue(5) });
