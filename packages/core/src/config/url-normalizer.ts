 ...
 *
 * Deduplication note: URLs that normalize to the same value are deduplicated
 * because a Map is used (last entry wins). This is expected behavior — if
 * the same logical seed appears twice in a list, it counts as one seed for
 * comparison purposes.
 */
