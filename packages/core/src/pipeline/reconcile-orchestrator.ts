// packages/core/src/pipeline/reconcile-orchestrator.ts
import { createLoggerWithContext } from '@spatula/shared';
import type { JobConfig, ReconciliationConfig } from '../index.js';
import type { ExtractionWithSource } from '../index.js';
import type {
...
/**
 * Pure reconciliation logic. No BullMQ/Redis references.
 *
 * Loads extractions, enriches with page metadata, reconciles,
 * persists entities with source links, persists source trust,
 * updates job status, and publishes events.
 */
export async function processReconciliation(
  input: ReconciliationInput,
  deps: ReconcileOrchestratorDeps,
): Promise<PipelineReconciliationResult> {
  const { jobId, tenantId } = input;
  const logger = createLoggerWithContext('reconcile-orchestrator', { jobId, tenantId });
  ...
  try {
    ...
    logger.error({ jobId, error }, 'reconciliation failed');
    ...
    // Don't rethrow -- failed reconciliation shouldn't crash the caller
    return { entitiesCreated: 0, actionsGenerated: 0 };
  }
}
