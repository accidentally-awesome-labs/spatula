...
```typescript
import type { JobConfig, FieldDef } from '@accidentally-awesome-labs/spatula-core-types';
import { ErrorCode, JobConfigSchema } from '@accidentally-awesome-labs/spatula-core-types';
```

> **Note** This package is **type-only** in spirit. Importing runtime values for use as runtime values is allowed by the package itself, but the Spatula monorepo enforces a stricter convention via ESLint (`eslint.config.mjs`): value imports from `@accidentally-awesome-labs/spatula-core-types` are blocked across the monorepo (`allowTypeImports: true`). Use `@accidentally-awesome-labs/spatula-shared` for runtime `ErrorCode` / `STATUS_MAP`, or `@accidentally-awesome-labs/spatula-core` for the zod schemas. zod schemas exported here are SOURCE-OF-TRUTH; consumers may use them at runtime, but the package itself does not depend on zod at runtime — it declares zod as a peer.
