  ...
  describe('aggregateByTenant', () => {
    ...
    it('calls db.select() and returns correct shape with canned data', async () => {
      ...
      const limit = vi.fn().mockResolvedValue([
        { jobId: 'job-1', tokens: 300, costUsd: 0.003 },
      ]);
      ...
      const groupBy2 = vi.fn().mockResolvedValue([
        { purpose: 'extraction', tokens: 300, costUsd: 0.003 },
      ]);
      const where2 = vi.fn().mockReturnValue({ groupBy: groupBy2 });
      const from2 = vi.fn().mockReturnValue({ where: where2 });

      const groupBy1 = vi.fn().mockResolvedValue([
        { model: 'anthropic/claude-3-haiku', tokens: 300, costUsd: 0.003 },
      ]);
      const where1 = vi.fn().mockReturnValue({ groupBy: groupBy1 });
      const from1 = vi.fn().mockReturnValue({ where: where1 });

      const totalsWhere = vi.fn().mockResolvedValue([
        { totalTokens: 300, totalCostUsd: 0.003 },
      ]);
      ...
      const chainDb = { select: selectFn, insert: vi.fn().mockReturnValue({ values: vi.fn().mockReturnValue({ returning: vi.fn() }) }) } as any;
