...
describe('createDatabasePool', () => {
  ...
  it('throws on invalid DB_POOL_MAX', async () => {
    process.env.DATABASE_URL = 'postgresql://test@localhost/db';
    process.env.DB_POOL_MAX = 'abc';
    const { createDatabasePool } = await import('../../src/connection.js');
    expect(() => createDatabasePool()).toThrow('DB_POOL_MAX must be a positive integer');
  });

  it('throws on invalid DB_POOL_IDLE_TIMEOUT', async () => {
    process.env.DATABASE_URL = 'postgresql://test@localhost/db';
