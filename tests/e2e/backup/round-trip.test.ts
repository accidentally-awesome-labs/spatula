...
describe('Backup → restore round-trip (DEPLOY-05)', () => {
  it(
    'proves row-count + content-hash parity after pg_dump → restore',
    async (ctx) => {
      ...
      for (const table of SEEDED_TABLES) {
        const result = await db.execute(
          sql.raw(
            `SELECT COUNT(*) AS n FROM ${table} WHERE tenant_id = '${tenantId}'::uuid`,
          ),
        );
      ...
      try {
        ...
        for (const row of contentStoreAfter.rows as Array<{ key: string; content: string }>) {
          ...
          expect(restoredHash, `Content-hash mismatch for key: ${row.key}`).toBe(
            originalBlob!.hash,
          );
        }
